def test_func(cats_dict, cur):

    import psycopg2
    import pandas as pd

    # Check this out for how to set up database
    # https://www.digitalocean.com/community/tutorials/how-to-use-postgresql-with-your-django-application-on-ubuntu-20-04

    # Query the table
    cats_dict['var_ind'] = cats_dict['var_ind'][0:cats_dict['var_ind'].find(':')]
    cats_dict['var_ind'] = "DP05_0002PE"
    query = """SELECT """+cats_dict['var_ind']+""" FROM testtable"""

    cur.execute(query)
    val = cur.fetchall()
    newlist = []
    for x in val:
        newlist.append(list(x)[0])
